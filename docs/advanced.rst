...
For RAUC's test suite, a PKI with intermediate CAs and separate sub-trees for
development and release is generated by the script
``test/openssl-ca-create.sh``.
This PKI is intended to exercise many different code paths for testing and is
...
.. warning::
   ...
   If you have to do this, make sure that only signatures created by the
   intended certificates are accepted by RAUC in your configuration.
...
.. note::
   ...
   Polling is currently available at a preview level.

   This means that we want to allow users to try out this new feature and report
   their experiences, but also that your use case may not be fully supported yet.
   Depending on the feedback, we may need to change behaviour in ways that require
   corresponding changes to your ``system.conf``
   So, try it out and report back `using GitHub discussions
   <https://github.com/rauc/rauc/discussions/categories/general>`_ or
   ...
   Note that although the basic polling functionality is implemented, there are
   still a few gaps:

   * Show polling status information from D-Bus in the CLI.
   * Client-side phased/scheduled rollouts based on time windows, instead of
     using server-side logic.
   * Limit polling to specific windows (e.g. during the night).
   * Reject bundles based on age (based on limits from both system
     configuration and manifest).
   * Reconfiguration of the URL via D-Bus.
   * Support for :ref:`event logging <sec-advanced-event-log>`.

   See `GitHub issue 1114 <https://github.com/rauc/rauc/issues/1114>`_ for more
   details.
...
simple server-side script with HTTP redirect
  The script evaluates the HTTP headers sent by RAUC together with local
  information about the available update bundles to select one for
  installation.
...
The service only starts polling after the currently running slot is marked as
good to avoid installing to the only remaining good slot.
Currently, this means that after a service restart, you need mark it as good
again to restart polling.
...
be configured via ``candidate-criteria`` in the ``[polling]`` section.
...
.. code-block:: cfg
   ...
   [polling]
   url=https://example.com/stable/my_product.raucb
...
The :ref:`gdbus-property-de-pengutronix-rauc-Poller.Status` contains all
information about the polled bundle in the ``bundle`` and ``manifest``
elements.
...
information in your application, pass the *manifest hash* (from
``manifest.manifest-hash`` in the Status property) back to RAUC by calling the
InstallBundle method with the ``require-manifest-hash`` :ref:`argument
<gdbus-method-de-pengutronix-rauc-Installer.InstallBundle.args.require-manifest-hash>`.
...
If the bundle was replaced by a different (but correctly signed) bundle, this
is detected by comparing the manifest hashes and the installation is aborted.
...
* RAUC polls the URL every 4 hours as long as no error occurs.
* Every time a poll attempt fails, the interval is increased by 4 hours, until
  it reaches 24 hours.
* If an attempt is successful again, the interval is reduced to the initial 4
  hours.
